...
export default function TransactionListScreen(): React.ReactElement {
  ...
  const routeParams = route.params as
    ...
    | undefined;
  const [filterMode, setFilterMode] = useState<'month' | 'day' | 'search'>('month');
  ...
  const handleSearchChange = useCallback((text: string) => {
    setSearchQuery(text);
    setSelectedDayDate(null);
    setFilterMode(text.trim() ? 'search' : 'month');
  }, []);
  ...
  const netTotal = useMemo(() => {
    ...
    if (filterMode === 'search' && searchQuery.trim()) {
      list = transactions.filter((t) => matchesSearch(t, searchQuery));
    } else if (filterMode === 'day' && selectedDayDate) {
      const startOfDay = new Date(selectedDayDate);
      startOfDay.setHours(0, 0, 0, 0);
      const endOfDay = new Date(selectedDayDate);
      endOfDay.setHours(23, 59, 59, 999);
      list = transactions.filter((t) => t.date >= startOfDay && t.date <= endOfDay);
    }
    return list.reduce((sum, t) => {
      return sum + (t.type === 'income' ? t.amount : -t.amount);
    }, 0);
  }, [transactions, filterMode, selectedDayDate, searchQuery]);
  ...
  const summaryLabel =
    filterMode === 'search' && searchQuery.trim()
      ? `Net total · "${searchQuery.trim()}":`
      : filterMode === 'day' && selectedDayDate
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        <MonthCalendar
          ...
          selectedDay={filterMode === 'day' && selectedDayDate ? selectedDayDate.getDate() : null}
