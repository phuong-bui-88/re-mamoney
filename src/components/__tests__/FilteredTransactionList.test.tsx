...
describe('FilteredTransactionList', () => {
  ...
  describe('searchQuery filtering', () => {
    ...
    it('intersects selectedDate and searchQuery when both are provided', () => {
      useTransactionStore.setState({
        transactions: [
          mockTransaction({ id: 'i1', description: 'Chè mè đen', date: new Date(2026, 7, 4, 9) }),
          mockTransaction({ id: 'i2', description: 'Chè chuối', date: new Date(2026, 7, 20, 9) }),
          mockTransaction({ id: 'i3', description: 'Kẹo', date: new Date(2026, 7, 4, 10) }),
        ],
      });
      ...
      expect(screen.getByText('Chè mè đen')).toBeTruthy();
      expect(screen.queryByText('Chè chuối')).toBeNull();
      expect(screen.queryByText('Kẹo')).toBeNull();
    });
